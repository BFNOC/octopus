...
func listGroupPresets(c *gin.Context) {
	...
	if err != nil {
		writeGroupPresetError(c, codeGroupPresetListFailed, "group preset list failed", err)
		return
	}
...
func createGroupPreset(c *gin.Context) {
	...
	if err != nil {
		writeGroupPresetError(c, codeGroupPresetCreateFailed, "group preset create failed", err)
		return
	}
...
func createBlankGroupPreset(c *gin.Context) {
	...
	if err != nil {
		writeGroupPresetError(c, codeGroupPresetCreateBlankFailed, "group preset create blank failed", err)
		return
	}
	resp.Success(c, preset)
}
...
func cloneGroupPreset(c *gin.Context) {
	...
	if err != nil {
		writeGroupPresetError(c, codeGroupPresetCloneFailed, "group preset clone failed", err)
		return
	}
	resp.Success(c, preset)
}
...
func activateGroupPreset(c *gin.Context) {
	...
	if err := op.GroupPresetActivate(id, c.Request.Context()); err != nil {
		writeGroupPresetError(c, codeGroupPresetActivateFailed, "group preset activate failed", err)
		return
	}
...
func updateGroupPreset(c *gin.Context) {
	...
	if err != nil {
		writeGroupPresetError(c, codeGroupPresetUpdateFailed, "group preset update failed", err)
		return
	}
	resp.Success(c, preset)
}
...
func deleteGroupPreset(c *gin.Context) {
	...
	if err := op.GroupPresetDelete(id, c.Request.Context()); err != nil {
		writeGroupPresetError(c, codeGroupPresetDeleteFailed, "group preset delete failed", err)
		return
	}
...
func setGroupPin(c *gin.Context) {
	...
	if err := op.GroupSetPinned(id, *req.Pinned, c.Request.Context()); err != nil {
		writeGroupPresetError(c, codeGroupPinFailed, "group pin failed", err)
		return
	}
	resp.Success(c, "group pin updated")
}

func writeGroupPresetError(c *gin.Context, code string, message string, err error) {
	status := groupPresetErrorStatus(err)
	resp.ErrorWithAppError(c, status, groupError(code, message, err).WithStatus(status))
}

func groupPresetErrorStatus(err error) int {
	message := strings.ToLower(err.Error())
	switch {
	case strings.Contains(message, "not found"):
		return http.StatusNotFound
	case strings.Contains(message, "cannot delete active"),
		strings.Contains(message, "already exists"),
		strings.Contains(message, "duplicate"),
		strings.Contains(message, "unique"):
		return http.StatusConflict
	case strings.Contains(message, "required"),
		strings.Contains(message, "missing channels"),
