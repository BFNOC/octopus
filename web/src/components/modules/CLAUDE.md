...
| `site/` | `index.tsx`, `CheckinPanel.tsx`, `checkin-status.ts`, `ui-store.ts`, `site-message.ts` | 站点列表、签到面板、消息显示 |
...
| `toolbar/` | `index.tsx`, `search-store.ts`, `view-options-store.ts` | 跨模块复用的搜索/视图工具栏 |
